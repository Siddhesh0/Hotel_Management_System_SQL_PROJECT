...
('Mohit', 'Desai', 1, 52000, '2024-02-02', 'mohit.desai@example.com');
...
('Kunal', 'Singh', 7, 68000, '2024-03-03', 'akunal.singh@example.com');
...
select d.departmentid, d.departmentname, sum(e.salary) as totalsalary from employees e
...
-- department. 
select departmentname, count(*) as employee_count from departments
...
select departmentid, DateOfJoining, count(*) as employeescount from employees
where DateOfJoining like '2024%'
...
select departmentid, salary, count(*) as employeescount from employees
where salary between 50000 and 70000
...
select location, count(departmentname) as count_department
from departments
group by Location
...
-- in each city. 
select location, sum(annualbudget) as totalbudget, departmentname from departments
...
-- manager. 
SELECT Headofdepartment AS ManagerName,
...
-- city. 
select location, departmentname, count(annualbudget) as budgetcount from departments
group by location, departmentname
order by budgetcount;
...
select concat(departmentname, '  ', location) as departmentloc from departments;



-- Situation -3: To prepare a personalized email greeting, concatenate "Hello " with the employee's 
-- name and a comma. 
select concat('Hello', '  ', firstname,',') as mailgreeting from employees;
...
-- each employee. 
select concat(street, '  ', location, ' ', postal_code) as complete_address from employees;
...
-- with their position. 
select concat(departmentname, ' | ', Headofdepartment) as  Role from departments;


-- Situation -9: To display the employee's work status, concatenate their job title and current 
-- project. 
select concat(departmentname, ' | ', project) as  employees_work from departments;



-- Situation -10: For an annual report, concatenate the year and employee’s first name to create a 
-- unique report reference code. 
select concat(dateofjoining, ' | ', firstname) as  employeeYear from employees;
...
delete from employees
where dateofjoining<'2020-01-01' and project is null;
...
delete from employees
where Hired = 'North' and lastworkingdate < '2023-01-01';
...
alter table employees
modify column Status varchar(25) default 'Active';
...
create table employees_before_2020 as
select * from employees
